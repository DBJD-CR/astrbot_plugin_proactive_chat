...
>
> 这是我在Github上的首个仓库，也是第一次以开发者的身份参与到开源社区中，如果存在做的不好的地方还请理解。
> 
...
> [!NOTE]
> 根据我对 v0.9.7 版本的简单测试，目前插件的主要功能都能正常运转，不过需要有好的提示词进行配合，才能获得理想的主动消息效果。
> 
> v0.9.7 版本下，主动消息是通过发出一条**模拟的用户消息**来实现的，因此需要配合高质量的提示词，才能避免模型的回复“出戏”。
> 
> 理论上，理想的主动消息应该由系统下发，指挥模型去生成一段主动消息。这样提示词只需要简单配置也能达到理想的效果，也足够纯净。
> 
> 但这仅是我个人的理论猜想，而且沿用了 Kourichat 中的设计理念，这可能不适用于 AstrBot 的架构。
> 
> 我之前同样尝试过让 AI 往这个方向去开发，但最后都失败了，什么“伪事件注入”的看得我头大。
> 
> 但这也不一定就说明这条路径是死路，可能只是因为我和 AI 的能力不足罢了，还望各位大佬指点迷津。
> 
...
1.  **下载插件**: 通过 AstrBot 的插件市场下载。或从本 GitHub 仓库下载 `astrbot_plugin_proactive_chat` 的 `.zip` 文件，在 AstrBot WebUI 中的插件页面中选择 `从文件安装` 。
...
- **`主动消息动机 (内心独白)`**: **【核心】** 这是插件的灵魂。在这里，你需要以**第一人称（我/主人）**的口吻，编写一段话，来告诉你的 Bot 在发起主动消息时应该想什么、做什么。
